...
(() => {
 ...
 function create({T,objects,batches,manifest}){
  ...
  function tick(dt){if(!paused)elapsed+=dt;
  ...
  return{tick,setPaused(v){paused=!!v;},state(){return{paused,elapsed,harvestProgress:harvest,cutInstances:crop.filter(c=>c.cut).length,vehicles:rules.filter(r=>['shuttle','rail','moored','harvest','orbit-flight'].includes(r.type)).map(r=>({id:r.id,position:objects.get(r.id)?.position.toArray()}))};},dispose(){crop.length=0;}};
 }
