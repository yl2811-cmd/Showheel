...
(() => {
  ...
  function create({container,node,epoch:initialEpoch=3094,onState=()=>{},onSelect=()=>{},onNavigate=()=>{}}) {
    ...
    function search(query){const q=String(query||'').trim().toLocaleLowerCase();return[objectFor(hostId),objectFor(planetId)].filter(o=>!q||[o.name,o.id,o.spectralType,node.name].some(v=>(String(v||'')+' '+(window.ATLAS_I18N?.english(v)||'')).toLocaleLowerCase().includes(q)));}
