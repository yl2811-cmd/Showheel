...
function create(T,renderer,scene,camera,onState){
 ...
 function boot(state){worker?.terminate();worker=new Worker('orun-cloud-worker.js');terrain=state.landforms||state;worker.postMessage({type:'init',state:{seed:state.seed,climate:state.climate,landforms:{h:terrain.h,n:terrain.n,min:terrain.min,step:terrain.step}}});worker.onmessage=e=>{if(e.data.id!==requestId||disposed)return;busy=false;if(e.data.error){lastStats={...lastStats,error:e.data.error};onState(lastStats);return;}if(inFlightKey!==revisionKey){dirty=true;return;}install(e.data);};worker.onerror=e=>{busy=false;lastStats={...lastStats,error:e.message};onState(lastStats);};}
